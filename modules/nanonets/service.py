...
class NanonetsExtractor:
    ...
    def __init__(self, prompt: str = PROMPT):
        self.nanonets_client = OpenAI(
            api_key=os.getenv("NANONETS_API_KEY"),
            base_url=os.getenv("NANONETS_API_URL"),
        )
        self.nanonets_model_id = os.getenv("NANONETS_MODEL_ID")
        self.nanonets_prompt = prompt

    def extract(self, pdf_bytes: bytes) -> str:
        """
        Extract text from PDF bytes using Nanonets.
        ...
        print(f"Extracting text from {total_pages} pages using Nanonets...")
    ...
    def _read_page_as_markdown(self, page_image_b64: str) -> str:
        """
        Use Nanonets to extract markdown text from a base64-encoded page image.
        ...
        response = self.nanonets_client.chat.completions.create(
            model=self.nanonets_model_id,
            temperature=0,
            messages=[
                {
                    "role": "user",
